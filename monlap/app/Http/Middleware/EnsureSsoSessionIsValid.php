...
class EnsureSsoSessionIsValid
{
            ...
            if ($token) {
                ...
                if (!$lastChecked || (microtime(true) - $lastChecked) > 2.0) {
                    ...
                    if (!$isValid) {
                        ...
                        return redirect('/')->withErrors(['error' => 'Sesi SSO Anda telah berakhir atau Anda telah logout dari Portal SSO.']);
                    }
    ...
    {
        try {
            $ssoBaseUrl = rtrim(config('services.sso.url', 'http://localhost/sso/public'), '/');
