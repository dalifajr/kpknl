...
if (!isset($_POST['selected_ids']) || count($_POST['selected_ids']) === 0) {
    echo "<script>alert('Tidak ada risalah yang dipilih!'); window.location.href='pinjam_risalah_tap.php';</script>";
    exit();
}
...
try {
    $sqlSelect = "SELECT * FROM risalah_tap WHERE id IN ($idList) FOR UPDATE";
    $res = $conn->query($sqlSelect);

    $stmtUpd = $conn->prepare("UPDATE risalah_tap SET status='sedang_dipinjam' WHERE id=? AND status!='sedang_dipinjam'");
    ...
    echo "<script>alert('$msg'); window.location.href='pinjam_risalah_tap.php';</script>";

} catch (Exception $e) {
    $conn->rollback();
    echo "<script>alert('Error: ".$e->getMessage()."'); window.location.href='pinjam_risalah_tap.php';</script>";
}
