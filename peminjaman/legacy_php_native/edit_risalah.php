...
$query = mysqli_query($conn, "SELECT * FROM risalah_minuta WHERE id = '$id'");
...
if (isset($_POST['update'])) {
    $box   = $_POST['box'];
    $lemari= $_POST['lemari'];
    $link  = $_POST['link_erisalah'];

    $update = mysqli_query($conn, "UPDATE risalah_minuta SET 
        box = '$box',
        lemari = '$lemari',
        link_erisalah = '$link'
        WHERE id = '$id'
    ...
    if ($update) {
        echo "<script>alert('Data berhasil diupdate!'); window.location='cek_data_risalah.php';</script>";
    } else {
...
<title>Edit Data Risalah Minuta</title>
<style>
    body {
...
<body>
<div class="container">
    <h2>Edit Data Risalah Minuta</h2>
    <form method="POST">
        ...
        <label>Link E-Risalah:</label>
        <input type="url" name="link_erisalah" value="<?= $data['link_erisalah'] ?>" placeholder="Masukkan link dokumen (Google Drive / PDF)">
