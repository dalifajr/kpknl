...
// Query gabungan risalah_minuta + peminjaman
$sql = "
    SELECT r.id, r.no_risalah, r.tgl_risalah, r.nama_pelelang, r.pemohon_lelang, 
           r.box, r.lemari, r.status, p.nama_peminjam
    FROM risalah_minuta r
    LEFT JOIN peminjaman p ON r.no_risalah = p.no_risalah AND r.status = 'sedang_dipinjam'
    WHERE 1=1
";
...
// Urutan: nomor risalah besar → kecil, tahun terbaru → lama, lalu tanggal terbaru → lama
$sql .= "
 ORDER BY 
   r.tgl_risalah DESC,
   CAST(SUBSTRING_INDEX(r.no_risalah, '/', 1) AS UNSIGNED) DESC
";
...
<head>
    <meta charset="UTF-8">
    <title>Pinjam Risalah Batal</title>
...
<div class="container">
    <div class="header">
        ...
        <form method="GET" class="search-form">
            ...
            <a href="pinjam_risalah.php" class="reset-btn">Reset</a>
        </form>
        <div class="kembali">
    ...
    <form method="POST" action="alasan_peminjaman.php">
        <!-- Tombol pindah ke atas kanan -->
