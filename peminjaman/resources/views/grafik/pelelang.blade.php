...
@section('title', 'Grafik Produktivitas Pelelang')
...
<div class="card" style="max-width: 960px; margin: 0 auto 24px;">
    <div class="card-header">
        <div>
            <h1 class="card-title">GRAFIK PELELANG</h1>
            <p style="color: var(--text-muted); font-size: 0.875rem;">Analisis statistik pelelangan bulanan dan tahunan per Pejabat Lelang.</p>
        </div>
    </div>
    
    <form method="GET" action="{{ route('grafik.pelelang') }}" style="background: var(--bg-subtle); padding: 22px; border-radius: var(--radius-md); border: 1px solid var(--border-color); margin-bottom: 24px;">
        <div class="form-group">
            <label class="form-label" for="pelelang">Pilih Nama Pejabat Lelang:</label>
            <select name="pelelang" id="pelelang" class="form-control" required>
                <option value="">-- Pilih Nama Pejabat Lelang --</option>
                @foreach ($pelelangList as $name)
                    <option value="{{ $name }}" {{ $pelelang === $name ? 'selected' : '' }}>
                        {{ $name }}
                    </option>
                @endforeach
            </select>
    ...
    @if ($jenis === 'bulanan' && !empty($dataBulanan))
        <div style="text-align: center; margin: 20px 0 10px;">
            <h3 style="font-size: 1.25rem; font-weight: 800; color: var(--primary-accent); letter-spacing: -0.01em;">
                GRAFIK BULANAN {{ strtoupper($pelelang) }}
            </h3>
        </div>
        
        <div class="year-pills-container">
            @foreach ($dataBulanan as $tahun => $bulanan)
                <button type="button" class="year-pill btn-tahun-pelelang" onclick="showChart({{ $tahun }}, this)">
                    {{ $tahun }}
                </button>
            @endforeach
        </div>
    ...
    @elseif ($jenis === 'tahunan' && !empty($dataTahunan))
        <div style="text-align: center; margin: 20px 0 15px;">
            <h3 style="font-size: 1.25rem; font-weight: 800; color: var(--primary-accent); letter-spacing: -0.01em;">
                GRAFIK TAHUNAN {{ strtoupper($pelelang) }}
            </h3>
        </div>
    ...
    @elseif ($pelelang && empty($dataBulanan) && empty($dataTahunan))
        <div style="padding: 30px; text-align: center; color: var(--text-muted);">
            Tidak ditemukan data risalah lelang untuk Pejabat Lelang ini.
        </div>
    @endif
</div>
@endsection
    ...
    function showChart(tahun, el) {
        ...
        chartInstance = new Chart(ctx, {
            ...
            data: {
                labels: bulanLabels,
                datasets: [{
                    label: 'Jumlah Data Risalah ' + tahun,
                    data: dataBulan,
                    borderColor: '#0284c7',
        ...
        const buttons = document.querySelectorAll('.btn-tahun-pelelang');
