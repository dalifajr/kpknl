...
@section('title', 'Edit Aplikasi - ' . $application->name)

@section('content')
<div class="d-flex align-items-center justify-content-between mb-4">
    <div>
        <h3 class="mb-1 fw-bold text-dark">Edit Aplikasi: {{ $application->name }}</h3>
        <p class="text-muted mb-0">Perbarui detail nama, URL, callback, atau status aplikasi ini.</p>
    </div>
    ...
    <form action="{{ route('admin.applications.update', $application->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row g-4">
            <div class="col-md-6">
                ...
                <input type="text" name="name" id="name" class="form-control m3e-input rounded-3 py-2" value="{{ old('name', $application->name) }}" required>
            </div>

            <div class="col-md-6">
                <label for="slug" class="form-label fw-semibold text-dark fs-7">Slug Kode Aplikasi <span class="text-danger">*</span></label>
                <input type="text" name="slug" id="slug" class="form-control m3e-input rounded-3 py-2" value="{{ old('slug', $application->slug) }}" required>
            </div>

            <div class="col-12">
                <label for="description" class="form-label fw-semibold text-dark fs-7">Deskripsi Singkat Aplikasi</label>
                <textarea name="description" id="description" rows="2" class="form-control m3e-input rounded-3 py-2">{{ old('description', $application->description) }}</textarea>
            </div>

            <div class="col-md-6">
                <label for="url" class="form-label fw-semibold text-dark fs-7">URL Utama Aplikasi <span class="text-danger">*</span></label>
                <input type="url" name="url" id="url" class="form-control m3e-input rounded-3 py-2" value="{{ old('url', $application->url) }}" required>
            </div>
            ...
            <div class="col-md-6">
                ...
                <input type="url" name="redirect_uri" id="redirect_uri" class="form-control m3e-input rounded-3 py-2" value="{{ old('redirect_uri', $application->redirect_uri) }}" required>
            </div>
            ...
            <div class="col-md-6">
                ...
                <small class="text-muted fs-8 mt-1 d-block">Ganti file JPG, PNG, SVG (Maks. 2MB). Biarkan kosong jika tidak ingin mengubah.</small>
            </div>
            ...
            <div class="col-md-6">
                ...
                <div class="form-check form-switch mt-3 d-flex align-items-center">
                    <input class="form-check-input me-2 shadow-sm" type="checkbox" role="switch" name="status" id="status" value="active" {{ old('status', $application->status) == 'active' ? 'checked' : '' }}>
            ...
            <div class="col-12 mt-4 pt-3 border-top border-secondary-subtle d-flex gap-2">
                <button type="submit" class="btn btn-primary rounded-pill px-4 fw-bold shadow-sm">
                    <i class="fa-solid fa-check me-1"></i> UPDATE APLIKASI
                </button>
