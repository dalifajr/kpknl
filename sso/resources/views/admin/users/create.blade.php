...
@section('title', 'Tambah User Baru | SSO KPKNL Palembang')

@section('content')
<div class="d-flex align-items-center justify-content-between mb-4">
    <div>
        <h3 class="mb-1 fw-bold text-dark">Tambah User Baru</h3>
        <p class="text-muted mb-0">Buat akun baru dan tentukan hak akses role serta aplikasi yang di-assign.</p>
    </div>
    ...
    <form action="{{ route('admin.users.store') }}" method="POST">
        ...
        <div class="row g-3">
            <div class="col-md-6">
                ...
                <input type="text" name="name" id="name" class="form-control" placeholder="Contoh: Ahmad Rizky" value="{{ old('name') }}" required>
            </div>

            <div class="col-md-6">
                <label for="username" class="form-label fw-semibold text-dark fs-7">Username <span class="text-danger">*</span></label>
                <input type="text" name="username" id="username" class="form-control" placeholder="Contoh: ahmad_rizky" value="{{ old('username') }}" required>
                <small class="text-muted fs-8">Gunakan huruf kecil, angka, strip, atau garis bawah.</small>
            </div>

            <div class="col-md-6">
                <label for="email" class="form-label fw-semibold text-dark fs-7">Email Resmi <span class="text-danger">*</span></label>
                <input type="email" name="email" id="email" class="form-control" placeholder="ahmad@kpknl.go.id" value="{{ old('email') }}" required>
            </div>

            <div class="col-md-6">
                <label for="role_id" class="form-label fw-semibold text-dark fs-7">Role Akun <span class="text-danger">*</span></label>
            ...
            <div class="col-md-6">
                <label for="password" class="form-label fw-semibold text-dark fs-7">Password <span class="text-danger">*</span></label>
                <input type="password" name="password" id="password" class="form-control" placeholder="Minimal 8 karakter (NIST)" required>
                <small class="text-muted fs-8">Harus mengandung min 8 karakter, huruf besar, huruf kecil, angka, dan simbol.</small>
            </div>

            <div class="col-md-6">
                <label for="status" class="form-label fw-semibold text-dark fs-7">Status Akun <span class="text-danger">*</span></label>
                <select name="status" id="status" class="form-select" required>
                    <option value="active" {{ old('status', 'active') == 'active' ? 'selected' : '' }}>Active (Dapat Login)</option>
            ...
            <div class="col-12 mt-4">
                ...
                <div class="p-3.5 rounded-4 d-flex align-items-center justify-content-between flex-wrap gap-3" style="background-color: var(--md-sys-color-surface-container-high);">
                    ...
                    <button type="button" class="btn btn-tonal rounded-pill px-4 py-2" id="openAssignAppsModal">
                        <i class="fa-solid fa-pen-to-square me-1.5"></i> Pilih / Assign Aplikasi
                    </button>
                </div>
            ...
            <div class="col-12 mt-4 pt-3 border-top d-flex gap-2">
                <button type="submit" class="btn btn-primary rounded-pill px-4 fw-bold">
                    <i class="fa-solid fa-check me-1"></i> SIMPAN USER
                </button>
