...
class LoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        // Strict Maintenance Mode Check: Prevent non-superadmin and non-maintenance from logging in
        if (\App\Models\Setting::get('maintenance_mode', 'inactive') === 'active') {
            if (!$user || (!$user->isSuperadmin() && !$user->isMaintenance())) {
                ActivityLogService::log(
                ...
                return back()->withInput()->withErrors([
                    'login' => 'Sistem SSO sedang dalam pemeliharaan (Maintenance Mode). Hanya Superadmin dan Tim Maintenance yang diperbolehkan untuk masuk saat ini.',
                ]);
            }
        }
